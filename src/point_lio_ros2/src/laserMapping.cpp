...
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
    ...
    // last_timestamp_lidar = msg->header.stamp.sec; // NOTE: Replaced below
    last_timestamp_lidar = get_time_sec(msg->header.stamp);
    // cout << "last_timestamp_lidar: " << last_timestamp_lidar << endl;
...
bool sync_packages(MeasureGroup &meas) {
    // DEBUGG: {Print all states gated in this function}
    // std::cout << "[sync] imu_en=" << imu_en
    //         << " lidar_buf="  << lidar_buffer.size()
    //         << " time_buf="   << time_buffer.size()
    //         << " imu_buf="    << imu_deque.size()
    //         << " lidar_pushed=" << lidar_pushed
    //         << " last_imu_t=" << last_timestamp_imu
    //         << " lidar_end_t=" << lidar_end_time
    //         << " last_lidar_t=" << last_timestamp_lidar
    //         << std::endl;
    // imu_en: 0 to call below if. NOTE: 'imu_en' is 1 as set by launch 
    if (!imu_en) {
    ...
    // cout << "lidar_buffer.empty(): " << lidar_buffer.empty() << endl;
    // Will return 'false' if lidar_buffer is empty
    if (lidar_buffer.empty() || imu_deque.empty()) {
        // cout << "lidar_buffer is empty" << endl;
        return false;
    }

    /*** push a lidar scan ***/
    // cout << "lidar_pushed: " << lidar_pushed << endl; // lidar_pushed: false (as set)
    if (!lidar_pushed) {
        meas.lidar = lidar_buffer.front();
        if (meas.lidar->points.size() < 1) {
    ...
    if (last_timestamp_imu < lidar_end_time) {
        // cout << "last_timestamp_imu is < lidar_end_time" << endl;
        return false;
    }
    /*** push imu data, and pop from imu buffer ***/
    // cout <<"Reached last if: ImuProcess" << endl;
    if (p_imu->imu_need_init_) {
        cout << "Checked all above 'if'" << endl;
...
int main(int argc, char **argv) {
    ...
    cout << "Lidar topic: " << lid_topic.c_str() << endl;
    cout << "IMU topic: " << imu_topic.c_str() << endl;
...
//------------------------------------------------------------------------------------------------------
    ...
    while (rclcpp::ok()) {
        ...
        // cout << "Node initiated!" << endl;
        // cout << "imu_en: " << imu_en << endl;

        if (sync_packages(Measures)) {
            cout << "Sync packages" << endl;
            if (flg_first_scan) {
