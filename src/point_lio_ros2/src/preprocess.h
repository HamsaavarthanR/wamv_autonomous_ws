...
// NOTE: Inlcuded "WAMV LiDAR" specific definitions
enum LID_TYPE {
    AVIA = 1, VELO16, OUST64, HESAIxt32, UNILIDAR, WAMV_LIDAR
}; //{1, 2, 3, 4}
enum TIME_UNIT {
...
// Custom Wam-V LiDAR Point Type (with points timestamp and ring)
namespace wamv_lidar_ros {
    struct EIGEN_ALIGN16 Point {
        // std_msgs::msg::Header header; // Header to store timestamp and frame_id
        PCL_ADD_POINT4D; // This macro adds the x, y, z coordinates
        float intensity;
        // Additional fields for WAM-V LiDAR
        uint32_t ring;
        float time; // points (in seconds)

        EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    };
}  // namespace wamv_lidar_ros
// Register the Wam-V LiDAR Point Type with PCL
POINT_CLOUD_REGISTER_POINT_STRUCT(wamv_lidar_ros::Point,
                                    // (std_msgs::msg::Header, header, header)
                                    (float, x, x)
                                    (float, y, y)
                                    (float, z, z)
                                    (float, intensity, intensity)
...
private:
    ...
    // Wam-V LiDAR handler
    void wamv_lidar_handler(const sensor_msgs::msg::PointCloud2::SharedPtr &msg);
